...
# add your path to the mimic data
mimic_data_dir = 'mimic-data-preprocessing/mimic-iii/physionet.org/files/mimiciii/1.4/'
...
# load the subject ids of patients 
with open("mimic-data-preprocessing/patients_for_finetuning.json", "r") as fp:
    data = json.load(fp)
# patients with sepsis on admission need to be removed from the data
admission_sepsis = [int(x) for x in data["admission_patients_sepsis"] ] 
...
icu = icu[~icu["SUBJECT_ID"].isin(admission_sepsis)] #remove patients who had sepsis on admission
print(icu)
...
ev_vanc['NAME'] = 'Vancomycin'  #mistake 
...
invasive_ventilation = [225792] #mechanical ventilation in procedure_events 
...
# add the times the blood culture was taken. 
# original value is not present from the table, so 1 is assigned instead 

mb = pd.read_csv(mimic_data_dir+'MICROBIOLOGYEVENTS.csv',
    usecols = ["HADM_ID", 'SPEC_ITEMID', 'CHARTTIME', "SPEC_TYPE_DESC"])
mb = mb.loc[mb.HADM_ID.isin(icu.HADM_ID)]
mb.HADM_ID = mb.HADM_ID.astype(int)
mb = mb.loc[mb.CHARTTIME.notna()]
mb['ICUSTAY_ID'] = np.nan
mb['TABLE'] = 'microbiology'

blood_culture = [70012]
mb_culture = mb.loc[mb.SPEC_ITEMID.isin(blood_culture)]
mb_idx = (mb_culture.SPEC_TYPE_DESC=='BLOOD CULTURE')
mb_culture.loc[mb_idx, 'VALUENUM'] = 1
mb_culture.loc[mb_idx, 'VALUE'] = 1
mb_culture.loc[mb_idx, 'SPEC_TYPE_DESC'] = "Blood Culture"
mb_culture['VALUEUOM'] = None

mb_culture.rename(columns={'SPEC_TYPE_DESC':'NAME', "SPEC_ITEMID":"ITEMID"}, inplace=True)
events = pd.concat([events, mb_culture])

# List of all the antibiotics in mimmic
antibiotics = [ 225848, 225850, 225851, 225853, 225855, 225857, 225859, 225860,
                 225862, 225863, 225865, 225866, 225868, 225869,225871,225873, 225875,
                  225876, 225877, 225879, 225881, 225882, 225883, 225884, 225885, 225886,
                  225888, 225889, 225890, 225892, 225893, 225895, 225896, 225897, 225898 ]

# List of antibiotics that are used in cases with sepsis
sepsis_antibiotics_codes = [ 225855, 225893, 225851, 225853, 225798, 225859, 225879, 225883, 225876, 225842 ]         

# antibiotics from prescription
sepsis_antibiotics = ["Ceftriaxone", "Ciperacillin", "Cefepime", "Ceftazidime",
                      "Vancomycin HCL", "Ciprofloxacin", "Levofloxacin", "Meropenem",
                     "Imipenem", "Ampicillin", "Piperacillin-Tazobactam Na"]

prescription = pd.read_csv(mimic_data_dir+'PRESCRIPTIONS.csv',
    usecols = ["HADM_ID",'ICUSTAY_ID', 'DRUG', 'STARTDATE', 'ENDDATE',
               'ROUTE'])
...
new_prescription = prescription.loc[prescription.TD<=pd.Timedelta(1,'h')].drop(columns=['STARTDATE', 'TD'])
prescription = prescription.loc[prescription.TD>pd.Timedelta(1,'h')]
new_rows = []
for _,row in tqdm(prescription.iterrows()):
    hadm, icuid, drug, route, stm, td = row.HADM_ID, row.ICUSTAY_ID, row.DRUG, row.ROUTE, row.STARTDATE, row.TD
    td = td.total_seconds()/60
    num_hours = td // 60
    for i in range(1,int(num_hours)+1):
...
new_rows = pd.DataFrame(new_rows, columns=["HADM_ID",'ICUSTAY_ID', 'ENDDATE', 'DRUG', 'ROUTE'])
...
procedure = pd.read_csv(mimic_data_dir+'PROCEDUREEVENTS_MV.csv',
...
new_procedure = procedure.loc[procedure.TD<=pd.Timedelta(1,'h')].drop(columns=['STARTTIME', 'TD'])
procedure = procedure.loc[procedure.TD>pd.Timedelta(1,'h')]
new_rows = []
for _,row in tqdm(procedure.iterrows()):
    sub_id, hadm_id, itemid, icuid, stm, td = row.SUBJECT_ID, row.HADM_ID, row.ITEMID, row.ICUSTAY_ID, row.STARTTIME, row.TD
    td = td.total_seconds()/60
    num_hours = td // 60
    for i in range(1,int(num_hours)+1):
...
new_rows = pd.DataFrame(new_rows, columns=["SUBJECT_ID","HADM_ID","ITEMID",
                                           'ICUSTAY_ID', 'ENDTIME'])
...
# add the clinical notes to the features
notes = pd.read_csv(mimic_data_dir+'NOTEEVENTS.csv',
    usecols = ["HADM_ID","CHARTDATE", "CHARTTIME", "TEXT"])
notes['CHARTTIME']= notes['CHARTTIME'].fillna(notes["CHARTDATE"]+ " 00:00:00")
final_notes = notes.drop(columns=['CHARTDATE'])
final_notes["NAME"] = "Text"
final_notes.rename(columns={'TEXT':'VALUENUM'}, inplace=True)
final_notes = final_notes.loc[final_notes.HADM_ID.isin(icu.HADM_ID)]
final_notes.HADM_ID = final_notes.HADM_ID.astype(int)
final_notes = final_notes.loc[final_notes.CHARTTIME.notna()]
final_notes['ICUSTAY_ID'] = np.nan
final_notes['TABLE'] = 'noteevents'

events = pd.concat([events, final_notes])

# Save the data.
events.to_csv('mimic_iii_events_text.csv', index=False)
icu.to_csv('mimic_iii_icu_text.csv', index=False)